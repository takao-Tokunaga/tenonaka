...
final class TremorSensor: NSObject, ObservableObject {
    ...
    /**
     ...
     0.0013 は「置く」の**8.7倍上**、「机に構える」の**1.4倍下**。
     以前は 0.0008 だったが、置いたつもりの状態で判定が続くことがあったため上げた。
     これ以上上げると「机に肘をついて構える」= 正当な読み方が止まりはじめる。
     端末ごとに詰めたいときは `TREMOR_TEST=1` の画面にスライダがある。
     */
    var threshold: Double = 0.0013

    /**
     離してから止めるまでの猶予。

     掴み直す一瞬で止めないための遅らせだが、長いと**置いたのに文字が出続ける**。
     以前は3秒で、置いてから3秒ぶん先へ進んでしまっていた。
     1.5秒あれば持ち替えは吸収でき、置いたことも十分早く伝わる。
     */
    var releaseDelay: TimeInterval = 1.5
